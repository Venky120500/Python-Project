...
def purchase(List):
    ...
    print("\nHello " + a_name + "! Welcome to our Metal Paste Manufacturing Company .\nPlease select product as per your choice.")
    ...
    while flag.upper() == "Y":  # check and go if flag is 'Y' or 'y'.
        ...
        if product_name == L[0][0].upper() \
            ...
            print("sorry!! " + product + " is not available in our company.")
            print("\nChoose from following Material please!")
            print("--------------------------------------------")
    ...
    for keys in q.keys():
        if keys == L[0][0].upper():  # executes this operation if product is phone entered by customer.
            ...
            print("\nTotal cost for METAL PASTE: ", p_amount)
    ...
    for keys in q.keys():  # In this loop, write in a file only those product which is purchase by user.
        if keys == "METAL PASTE":
            file.write(
                str("\n" + str(keys) + " \t\t " + str(q['METAL PASTE']) + " \t\t " + str(L[0][1]) + " \t\t " + str(p_amount)))
        elif keys == "WOODEN FLOOR":
            file.write(str(
                "\n" + str(keys) + " \t\t " + str(q['WOODEN FLOOR']) + " \t\t " + str(L[1][1]) + " \t\t " + str(l_amount)))
        else:
            file.write(
                str("\n" + str(keys) + " \t\t " + str(q['WOODEN PUTT']) + " \t\t " + str(L[2][1]) + " \t\t " + str(h_amount)))
